...
export default class Hue {
  ...
  static async link(address: string): Promise<string> {
    const resp = await fetch(`http://${address}/api`, {
      method: 'POST',
  ...
  static async getNumLights(address: string, user: string): Promise<number> {
  ...
  static getUri(address: string, username: string, endpoint: string): string {
